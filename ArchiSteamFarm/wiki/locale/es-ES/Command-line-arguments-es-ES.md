...
Ten en cuenta que para un autoapagado correcto necesitas otras configuraciones - especialmente evitar `--process-required` y asegurarse que todos tus bots tengan `ShutdownOnFarmingFinished` en la propiedad `FarmingPreferences`. Por supuesto, el cierre automático solo es una posibilidad para esta característica, no un requisito, ya que también puedes usar esta opción junto con, por ejemplo `--process-required`, haciendo que tu sistema se mantenga activo infinitamente después de iniciar ASF.