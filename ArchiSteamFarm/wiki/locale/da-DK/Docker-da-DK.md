...
Og det er det, nu bruger din ASF-dockercontainer delt mappe med din lokale maskine i læse-skrivningstilstand, hvilket er alt hvad du har brug for til at konfigurere ASF. In similar way you can mount other volumes that you'd like to share with ASF, such as `/app/logs` or `/app/plugins/MyCustomPluginDirectory` (you don't want to override `/app/plugins` itself, since this way you'll remove plugins that ship with ASF by default).
