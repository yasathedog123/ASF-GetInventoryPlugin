...
**`Simple`** algorithm works best for accounts that have unrestricted card drops. 這是 ASF 使用的主要算法。 機械人檢測可用的遊戲，並逐個遊玩它們直到掛卡完成。 This is because card drops rate when farming more than one game is close to zero and totally ineffective.
