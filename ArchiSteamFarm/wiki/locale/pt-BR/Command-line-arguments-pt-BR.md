...
Tenha em mente que para o ASF desligar sozinho você precisa de outras configurações — evitando especialmente `--process-required` e garantir que todos os seus bots estejam seguindo `ShutdownOnFarmingFinished` em suas `FarmingPreferences`. Claro, o desligamento automático é apenas uma possibilidade para este recurso e não uma exigência, já que você também pode usar esta opção juntamente com `--process-required` fazendo seu sistema ficar ativo infinitamente após iniciar o ASF.